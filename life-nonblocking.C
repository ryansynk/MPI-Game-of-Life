 ...
 * Simulation is done in parallel using OpenMPI and non-blocking communication.
 * Run using
...
int main(int argc, char *argv[]) {
    ...
    // Initializations -- allocated by rank 0.
    int **life;
    int *life_vec;

    if (myrank == 0) {
        // Rank 0 sends board to other processes
        // Initialize life (full board)
        ...
        MPI_Request *board_send_requests = new MPI_Request[my_X_limit];
	    for (int i = 0; i < my_X_limit; i++) {
	        board_send_requests[i] = MPI_REQUEST_NULL;
	    }
	    ...
	    for (int i = 1; i < numprocs; i++) {
	        ...
	        for (int j = 0; j < my_X_limit; j++) {
                ...
                MPI_Isend(life[block_num + j], Y_limit, MPI_INT, i, 0, MPI_COMM_WORLD, &board_send_requests[j]);
	        }
	        MPI_Waitall(my_X_limit, board_send_requests, MPI_STATUSES_IGNORE);
                
	        // reset requests array for send to next process
	        for (int i = 0; i < my_X_limit; i++) {
	            board_send_requests[i] = MPI_REQUEST_NULL;
	        }
	    }
	    ...
	    delete[] board_send_requests;
    }

    if (myrank != 0) {
        // Other processes recieve initial sections of board
        MPI_Request *board_recv_requests = new MPI_Request[my_X_limit];
	    for (int i = 0; i < my_X_limit; i++) {
	        board_recv_requests[i] = MPI_REQUEST_NULL;
	    }
        for (int i = 0; i < my_X_limit; i++) {
            MPI_Irecv(mylife[i], Y_limit, MPI_INT, 0, 0, MPI_COMM_WORLD, &board_recv_requests[i]);
        }
        MPI_Waitall(my_X_limit, board_recv_requests, MPI_STATUSES_IGNORE);
     	delete[] board_recv_requests;
    }
    ...
    for (int numg = 0; numg < num_of_generations; numg++) {
    	...
    	MPI_Request left_send_request, left_recv_request;
        MPI_Request right_send_request, right_recv_request;
	    if (myrank == 0) {
	        // leftmost edge of board -- only needs to send and recieve last column
	        MPI_Isend(mylife[my_X_limit - 1], Y_limit, MPI_INT, 
                      myrank+1, 0, MPI_COMM_WORLD, &right_send_request);
	        MPI_Irecv((1 + my_previous_life[my_X_limit + 1]), Y_limit, MPI_INT, 
                      myrank+1, 0, MPI_COMM_WORLD, &right_recv_request);
            ...
            MPI_Irecv((1 + my_previous_life[0]), Y_limit, MPI_INT, 
                      myrank-1, 0, MPI_COMM_WORLD, &left_recv_request);
	        MPI_Isend(mylife[0], Y_limit, MPI_INT, myrank-1, 0, MPI_COMM_WORLD, 
                      &left_send_request);
	        ...
	        MPI_Isend(mylife[0], Y_limit, MPI_INT, myrank-1, 0, 
	        ...
	        MPI_Irecv((1 + my_previous_life[my_X_limit + 1]), Y_limit, MPI_INT, 
                      myrank+1, 0, MPI_COMM_WORLD, &right_recv_request);
	        MPI_Irecv((1 + my_previous_life[0]), Y_limit, MPI_INT, myrank-1, 
                      0, MPI_COMM_WORLD, &left_recv_request);
